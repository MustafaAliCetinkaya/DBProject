...
select job_id, avg(salary) as "average salary" from employees group by job_id order by avg(salary) desc ;
...
select * from employees where last_name LIKE '%a%a%';
 ...
 select max(salary), job_id from employees group by job_id order by max(SALARY) desc ;
